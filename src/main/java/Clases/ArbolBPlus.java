...
public class ArbolBPlus implements Serializable{ // Serializable = para guardar el objeto sin perder los datos del arbol del historial
    ...
    public PedidoClass buscarPorId(long id) {
        Nodo actual = raiz;

        // Bajamos por el árbol hasta llegar a una hoja
        while (!actual.esHoja) {
            int i = 0;
            while (i < actual.llaves.size() && id >= actual.llaves.get(i)) {
                i++;
            }
            actual = actual.hijos.get(i);
        }

        // Ya estamos en la hoja, buscamos la llave exacta
        for (int i = 0; i < actual.llaves.size(); i++) {
            if (actual.llaves.get(i) == id) {
                return actual.pedidos.get(i);
            }
        }
